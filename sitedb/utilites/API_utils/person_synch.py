...
def persons_analys(url, curs, session):
    """
    Сравнивает информацю о записях субьектов на сайте и в базе данных
    :param url: адрес сайта
    :param curs: курсор соединения к базе
    :return:
    """
    type_url = url + 'persons_api/'                          # сcылка на API - список объектов # отличие 1
    ...
    print(objs_json)
    db_objs = Tables("Людина", curs)  # отличие 2
    print(db_objs)
...
def add_new_obj(type_url, db_obj, session):
    ...
    pst = {
        "id": db_obj.Код_люд,  # 4.1 разница
        "name": db_obj.__dict__['ПІБ'],  # 4.2 разница
    }
