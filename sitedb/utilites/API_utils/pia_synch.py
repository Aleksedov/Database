...
def pia_analys(url, curs, session):
    """
    ...
    :return:
    """
    type_url = url + 'pia_api/'                          # сcылка на API - список объектов # отличие 1
    ...
    if cont:
        ...
        objs_json = {}
    print(objs_json)
    db_objs = Tables("Актори акту", curs)                         # отличие 2
    print(db_objs)
...
def add_new_obj(type_url, db_obj, session):
    ...
    pst = {
        "id": db_obj.Код,                                            # 4.1 разница
        "person": db_obj.__dict__['Людина'],                             # 4.2 разница
        "act": db_obj.__dict__['Акт'],                             # 4.2 разница
    }
    ...
    try:
        ...
        print("onnectionError(err, request=request)",pst, obj_request, timeout=1000)
