...
def depriving_analys(url, curs, session):
    """
    ...
    :return:
    """
    type_url = url + 'depriving_api/'                          # сcылка на API - список объектов # отличие 1
    ...
    if cont:
        ...
        objs_json = {}
    print(objs_json)
    db_objs = Tables("Місце позбавлення волі", curs)                         # отличие 2
    print(db_objs)
    ...
    if objs_json:
        ...
        for i_site in objs_json.items:
            ...
            if i_site not in db_objs.items:
                ...
                pare_keys = {
                    "condition": "місце перебування",
                    "status": "Статус перебування",
                    "overview": "примітки",
                    "persecution": "Переслідування",
                    "organisation": "Місце позбавлення волі",
                }
                ...
                if discr:
                    ...
                    pt = {
                        "id": "%s" % site_p.id,
                        "persecution": "%s" % site_p.persecution,
                        "organisation": "%s" % site_p.organisation,
                        "date": "%s" % site_p.date,
                    }
...
def add_new_obj(type_url, db_obj, session):
    ...
    pst = {
        "id": db_obj.Код_мпв,                                            # 4.1 разница
        "persecution": db_obj.__dict__['Переслідування'],                             # 4.2 разница
        "organisation": db_obj.__dict__['Місце позбавлення волі'],
        "date": trans_date(db_obj.__dict__['початок']),
    }
...
def main():
    ...
    curs = conn.cursor()
    # curs.execute("DROP TABLE 'Місце позбавлення волі'")
    username = 'admin'
    password = '111111111'
