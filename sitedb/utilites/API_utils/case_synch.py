...
def case_analys(url, curs, session):
    """
    Сравнивает информацю о записях кейсов на сайте и в базе данных
    :param url: адрес сайта
    :param curs: курсор соединения к базе
    :return:
    """
    type_url = url + 'cases_api/'                          # сcылка на API - список объектов # отличие 1
    ...
    db_objs = Tables("Кейс", curs)                         # отличие 2
    print(db_objs)
    db_objs.add_all_items()
    print("обьектов на сайте = ", len(objs_json.items), "обьектов в базе  = ", len(db_objs.items))
    print(objs_json.items[1].__dict__.keys())
    print(db_objs.items[1].__dict__.keys())
    for i_site in objs_json.items:
        ...
        if i_site not in db_objs.items:
            ...
            discr = compare_object(objs_json.items[i_site], db_objs.items[i_site]) # отличие 3 данные в функции
            if discr:
...
def compare_object(site_obj, db_obj):
    ...
    discr = {}
    if site_obj.name != db_obj.__dict__['Назва кейсу']:
        print("несовпадение имени ", site_obj.name, db_obj.__dict__['Назва кейсу'])
        discr["name"] = "%s" %db_obj.Назва
    if site_obj.overview != db_obj.__dict__['Опис кейсу']:
        print("несовпадение описания ", site_obj.overview, db_obj.__dict__['Опис кейсу'])
        discr["overview"] = "%s" %db_obj.__dict__['Опис кейсу']
    return discr
...
def add_new_obj(type_url, db_p, session):
    ...
    pst = {
        "id": "%s" %db_p.Код_кейс,                                          # 4.1 разница
        "name": "%s" %db_p.__dict__['Назва кейсу'],                         # 4.2 разница
    }
    if db_p.__dict__['Опис кейсу']:                                         # 4.3 все последующие поля
        pst["overview"] = db_p.__dict__['Опис кейсу']
